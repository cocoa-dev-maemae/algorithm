 ...
 */
public class DiceThrowMem {
    /**
     * Count ways
     * @param f: facse of dice
    ...
    public static long findWays(int f, int d, int s) {
        ...
        for (int i = 1; i <= d; i++) {
            // Iterate over sum
            for (int j = i; j <= s; j++) {
                // The result is obtained in two ways, pin the current dice and spending 1 of the
                // value,
                ...
                // combinations
                // that pin the remaining j-1's. But there is a way, when "j-f-1> = 0" we would be
                // adding
                // extra combinations, so we remove the combinations that only pin the extrapolated
                // dice face and
